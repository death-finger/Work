...
host_linux = open('host-linux.txt', 'r')
test = open('test', 'r')
...
def get_gen_stats(hostlist, time_period):
    for host in hostlist.readlines():
        ...
        # CPU Load
        if linux == 1:
            query_cpu_load_id = "SELECT itemid FROM items WHERE hostid='%s' AND name='Processor load (15 min average per core)'" % host_result
        else:
        ...
        # Free Memory
        if linux == 1:
            query_mem_free_id = ("SELECT itemid FROM items WHERE hostid='%s' AND name='Available memory'" % host_result)
        else:
        ...
        if mem_total == 0 or type(mem_total) == type(None):
            mem_rate = ''
        else:
        ...
        if linux == 1:
            print(host, online_rate, cpu_load, mem_free, mem_rate, sep="\n")
            result.write(
                "%s\n%s\n%s\n%s\n" % (online_rate, cpu_load, mem_free, mem_rate))
        else:
        ...
        if linux == 1:
            for i in item_list:
                if i[2][:13] == 'vfs.fs.size[/' and i[2][-6:] == 'pfree]':
                    disk_list.extend(i)
                elif i[2][:13] == 'vfs.fs.size[/' and i[2][-6:] == ',free]':
                    disk_list_GB.extend(i)
        else:
            for i in item_list:
                if i[2][:11] == 'vfs.fs.size' and i[2][-8:] == ':,pfree]':
                    disk_list.extend(i)
                if i[2][:11] == 'vfs.fs.size' and i[2][-7:] == ':,free]':
                    disk_list_GB.extend(i)

        while disk_list:
            item_id, item_name, item_key = disk_list[:3]
            gb_id, gb_name, gb_key = disk_list_GB[:3]
            disk_list = disk_list[3:]
            disk_list_GB = disk_list_GB[3:]
            ...
            # Free GB
            query_disk_usage = ("SELECT AVG(value)/1073741824 FROM history_uint WHERE itemid=%s and clock between %s"
                                % (gb_id, time_period))
            cursor.execute(query_disk_usage)
            disk_usage_gb = cursor.fetchone()[0]
            print(disk_usage_gb)
            result.write("%s\n" % disk_usage_gb)
